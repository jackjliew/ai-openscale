...
若為 {{site.data.keyword.composeForPostgreSQL}} 服務，則需要「付費」標準方案。只要使用信用卡轉換成付費帳戶，即可取得 $200 {{site.data.keyword.Bluemix_notm}} 信用額度。如果您已有付費帳戶，會有一個月針對您所支付的第一個 GB 儲存空間費用，一次性退款 $16 給您。{: tip} --->
...
- [佈建 Db2 Warehouse 服務 ![「外部鏈結」圖示](../../icons/launch-glyph.svg "「外部鏈結」圖示")](https://{DomainName}/catalog/services/db2-warehouse){: new_window} 如果您的帳戶沒有相關聯的服務：

  ![Db2 Warehouse](images/db2_warehouse.png)

- 為您的服務命名，選擇「入門」方案，並按一下「建立」按鈕。

### 將訓練資料上傳至 Db2 Warehouse
{: #gs-traindb2}

- 下載 [drug_train_data_updated.csv ![「外部鏈結」圖示](../../icons/launch-glyph.svg "「外部鏈結」圖示")](https://raw.githubusercontent.com/watson-developer-cloud/doc-tutorial-downloads/master/ai-openscale/drug_train_data_updated.csv) 檔。請務必將下載的檔案儲存成 .CSV 檔。

- 從 [IBM Cloud 主控台 ![「外部鏈結」圖示](../../icons/launch-glyph.svg "「外部鏈結」圖示")](https://{DomainName}){: new_window}，開啟您現有（或新建）的 Db2 Warehouse，並按一下左側畫面的「管理」，然後按一下「開啟」按鈕。

- 必要的話，請使用您 Db2 認證的「使用者名稱」和「密碼」來登入 Db2 Warehouse。

- 一旦開啟 Db2 Warehouse 之後，請按一下「功能表」按鈕，並從功能表中選取「載入」：

  ![「載入」功能表](images/db2_load.png)
