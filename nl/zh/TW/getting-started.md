...
- 佈建 {{site.data.keyword.Bluemix_notm}} 機器學習和儲存服務
- 設置 Watson Studio 專案，並建立、訓練及部署一個機器學習模型
- 配置及探索您模型的信任度、透明度和可解釋性
## 佈建必備的 {{site.data.keyword.Bluemix_notm}} 服務
{: #gs-prps}

除了 {{site.data.keyword.aios_short}} 之外，要完成此指導教學，您還需要下列帳戶和服務。
...
**重要事項**：為了達到最佳效能，建議在與 {{site.data.keyword.aios_short}} 相同的區域建立必備服務。若要檢視 {{site.data.keyword.aios_short}} 的可用位置，請參閱[服務可用性](/docs/resources?topic=resources-services_region)。
1.  使用您的 {{site.data.keyword.ibmid}} 登入 [{{site.data.keyword.Bluemix_notm}} 帳戶 ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://{DomainName}){: new_window}。
1.  對於尚未與您帳戶相關聯的下列每一項服務，請按一下此鏈結，提供服務名稱，選取 **Lite**（免費）方案，然後按一下**建立**按鈕來建立實例：
    - [Watson Studio ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://{DomainName}/catalog/services/watson-studio){: new_window}
      ![Watson Studio](images/watson_studio.png)

    - [Watson Machine Learning ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://{DomainName}/catalog/services/machine-learning){: new_window}
      ![機器學習](images/machine_learning.png)

    - [物件儲存體 ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://{DomainName}/catalog/services/cloud-object-storage){: new_window}
      ![物件儲存體](images/object_storage.png)
...
- [佈建 Db2 Warehouse 服務 ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://{DomainName}/catalog/services/db2-warehouse){: new_window}（如果您尚無任何服務與您的帳戶相關聯）：

  ![Db2 Warehouse](images/db2_warehouse.png)

- 提供服務名稱，選擇「入門」方案，然後按一下 **建立** 按鈕。

### 將訓練資料上傳至 Db2 Warehouse
{: #gs-traindb2}
...
- 瀏覽至訓練資料檔，或將它拖放至表單上的適當區域。按「下一步」。從載入目標清單中選取綱目；這通常是類似於 'DASH12345' 的格式。然後按一下右側的「新建表格」：![新建表格](images/new_table.png)

- 將表格命名為 HEART\_DRUG\_TRAINING，然後按一下「建立」按鈕：

  ![建立新表格](images/new_table_create1.png)

- 按「下一步」，以預覽資料。在預覽畫面上，將「分隔字元」欄位設為分號 (;)，並確定已勾選「第一列中的標頭」選項：![分隔字元和標頭](images/separator.png)
...
1.  下載 [credit_payload_data.json ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://raw.githubusercontent.com/watson-developer-cloud/doc-tutorial-downloads/master/ai-openscale/credit_payload_data.json) 檔。
...
1.  指定內含模型預測之回答的特性。（在您的資料庫中，表格中的哪一個直欄包含預測值或標籤？）在此案例中，模型將預測貸方風險，因此請選取**風險**直欄，並按**下一步**。
