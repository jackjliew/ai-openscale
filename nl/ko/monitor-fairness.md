...
- **훈련**: 모델을 훈련시키는 데 사용되는 훈련 데이터 레코드입니다.
   ...
   - 훈련 데이터 레코드의 수. 훈련 데이터는 한 번 읽혀지며 subscription/fairness_configuration 아래 분포가 저장됩니다. 분포를 계산하는 동안 훈련 데이터 레코드의 수를 찾아서 동일한 분포에 저장해야 합니다. 또한 훈련 데이터가 변경되면 즉 POST/data_distribution이 다시 작성되는 경우 fairness_configuration/training_data_distribution 아래에서 이 값을 업데이트해야 합니다. 메트릭을 전송하는 동안 이 값도 전송해야 합니다. 
