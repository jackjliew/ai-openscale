...
{{site.data.keyword.composeForPostgreSQL}} 서비스의 경우, **유료** 표준 플랜이 필요합니다. 신용카드를 사용하여 유료 계정으로 전환하는 것으로 $200 {{site.data.keyword.Bluemix_notm}} 신용을 얻을 수 있습니다. 이미 유료 계정인 경우, 한 달 동안 첫 GB의 스토리지에 대해 일회의 $16 요금 환불을 받을 수 있습니다.
{: tip}
...
- 서비스 이름을 제공하고 Entry 플랜을 선택한 다음 **작성** 단추를 클릭하십시오.

### 교육 데이터를 Db2 Warehouse로 업로드
{: #gs-traindb2}

- [drug_train_data_updated.csv ![External link icon](../../icons/launch-glyph.svg "External link icon")](https://raw.githubusercontent.com/watson-developer-cloud/doc-tutorial-downloads/master/ai-openscale/drug_train_data_updated.csv) 파일을 다운로드하십시오. 파일을 .CSV 파일로 다운로드하십시오.

- [IBM Cloud console ![External link icon](../../icons/launch-glyph.svg "External link icon")](https://{DomainName}){: new_window}에서 기존 또는 새로 작성된 Db2 Warehouse를 열고 왼쪽 패널에서 **관리**를 클릭한 다음 **열기** 단추를 클릭하십시오.

- 필요한 경우, Db2 인증 정보 '사용자 이름' 및 '비밀번호'를 사용하여 Db2 Warehouse에 로그인하십시오.

- 일단 Db2 Warehouse가 열리면 **메뉴** 단추를 클릭하고 메뉴에서 **로드**를 선택하십시오.

  ![Load Menu](images/db2_load.png)

- 교육 데이터 파일을 찾거나 이를 양식의 적절한 영역에 끌어다 놓으십시오. **다음**을 클릭하십시오. 로드 대상 목록에서 스키마를 선택하십시오. 이는 일반적으로 `DASH12345`와 같은 형식입니다. 그런 다음 오른쪽에서 **새 테이블**을 클릭하십시오.
