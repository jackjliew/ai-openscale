...
{{site.data.keyword.composeForPostgreSQL}} サービスの場合は、「有料」の Standard プランが必要です。クレジット・カードで有料アカウントに転換することによって、$200 の {{site.data.keyword.Bluemix_notm}} クレジットを入手できます。既に有料アカウントをお持ちの場合、最初の 1 ギガ・ストレージ (1 カ月) 分のコストに対して 1 回限りの払戻金 $16 を受け取ることができます。
{: tip}
...
### トレーニング・データを Db2 Warehouse にアップロードする
{: #gs-traindb2}

- [drug_train_data_updated.csv ![外部リンク・アイコン](../../icons/launch-glyph.svg "外部リンク・アイコン")](https://raw.githubusercontent.com/watson-developer-cloud/doc-tutorial-downloads/master/ai-openscale/drug_train_data_updated.csv) ファイルをダウンロードします。ダウンロードしたファイルは必ず .CSV ファイルとして保存します。

- 既存の (または新規作成した) Db2 Warehouse を [IBM Cloud コンソール ![外部リンク・アイコン](../../icons/launch-glyph.svg "外部リンク・アイコン")](https://{DomainName}){: new_window} から開き、左側パネルの「管理」をクリックしてから「開く」ボタンをクリックします。

- 必要に応じて、Db2 資格情報 `username` と `password` を使用して Db2 Warehouse にログインします。

- Db2 Warehouse が開いたら、「Menu」ボタンをクリックしてメニューから「Load」を選択します。

  ![ロード・メニュー](images/db2_load.png)

- トレーニング・データ・ファイルを参照するか、そのファイルをフォームの適切な領域にドラッグ・アンド・ドロップします。「次へ」をクリックします。ロード・ターゲットのリストから「スキーマ」を選択します。これは通常、`DASH12345` のような形式になります。次に右側の「New Table」をクリックします。
...
1.  以下の画面キャプチャーに似た画面が表示されるはずです。データのスコアリングに GUI 方式を使用するため、**「モニターの構成」**ボタンを選択するだけでこのセットアップは完了します。
...
1.  数値データまたはカテゴリー・データの場合、モニターを構成するためにはモデルのトレーニング・データに関する情報を入力する必要があります。**「モニターの手動構成」**を選択して、トレーニング・データの接続情報を入力します。
...
1.  モデルのトレーニングに使用する列を選択します。これは、要求でモデル・デプロイメントが予期するデータです。`_training` を除くすべてのデータ列は、モデルに対する入力データです。他のすべての入力を選択し、**「次へ」**をクリックします。
    ...
    ![説明可能性の入力](images/explain_inputs1.png)

1.  カテゴリー・データでは、元の値はテキスト値で今は整数になっている列を指定する必要があります。以下に示すように、値を選択します。

    ![説明可能性の入力](images/config_categories.png)
...
1.  選択を確認し、**「保存」**をクリックしてから**「OK」**をクリックします。
    ...
    バイアスが排除されたスコアリング・エンドポイントを示す以下のウィンドウが表示されます。このチュートリアルではデータのスコアリングに GUI 方式を使用し、CLI は使用しないので、先に進むには**「OK」**をクリックします。
...
1.  最後に、フィードバック・データを追加するかどうかのオプションが表示されます。フィードバック・データについては次のセクションで取り上げます。今回は**「フィードバック・データの追加」**ボタンをクリックしないで**「OK」**をクリックし、ウィンドウを閉じます。
    ...
    詳しくは、[正確度モニターの構成](/docs/services/ai-openscale?topic=ai-openscale-acc-monitor#acc-config)を参照してください。
...
1. 「インサイト」ページを表示して、デプロイ済みモデルのメトリックの概要を確認します。90% のしきい値を下回っている公平性メトリックまたは正確度メトリックのアラートを簡単に確認できます。
