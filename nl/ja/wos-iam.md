...
サービスのアクセス役割 ({{site.data.keyword.aios_short}} へのアクセスと REST API の呼び出しをユーザーに許可する役割) については 、{{site.data.keyword.aios_short}} は、上記の表にリストされているプラットフォーム管理の役割に従います。UI でユーザーの役割を割り当てる方法については、[リソースへのアクセスの管理](/docs/iam?topic=iam-iammanidaccser#iammanidaccser)を参照してください。
