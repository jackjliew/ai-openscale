...
- グラフは、バイアスがあることを示しています。準拠集団の予想結果の割合を基準とする年齢 18 歳から 23 歳の集団の予想結果の割合の比率が、しきい値を下回っているからです。 つまり、0.52/0.7 = 0.74 であり、しきい値 0.8 より小さい値となっています。
...
- *アクティブなバイアス緩和* - アクティブなバイアス緩和では、アプリケーションからバイアス緩和 REST API エンドポイントを利用できます。 この REST API エンドポイントは内部でモデルを呼び出し、その振る舞いを検査します。
