...
## Alternative Konfigurationsoption
{: #gs-module}

Anstatt {{site.data.keyword.aios_short}} durch Ausführen des folgenden Lernprogramms zu erkunden, können technische Benutzer ein Python-Modul installieren, das die Einrichtung und Konfiguration der erforderlichen Services automatisiert. Dieses Modul setzt voraus, dass Python 3 installiert ist, was das Managementsystem für pip-Pakete enthält. Anweisungen enthält [Python-Modul zum Konfigurieren von {{site.data.keyword.aios_short}}](/docs/services/ai-openscale?topic=ai-openscale-as-module) installieren.

Weitere Links zu Lernprogrammen enthält der Abschnitt [Zusätzliche Ressourcen](/docs/services/ai-openscale?topic=ai-openscale-arsc-ov).
...
- Bereitstellen von {{site.data.keyword.Bluemix_notm}} Machine Learning-Services und -Speicherservices
- Einrichten eines Watson Studio-Projekts und Erstellen, Trainieren und Bereitstellen eines Machine Learning-Modells
- Vertrauen, Transparenz und Erklärbarkeit für Ihr Modell konfigurieren und erkunden
## Bereitstellen vorausgesetzter {{site.data.keyword.Bluemix_notm}}-Services
{: #gs-prps}

Abgesehen von {{site.data.keyword.aios_short}} benötigen Sie zum vollständigen Ausführen dieses Lernprogramms die folgenden Konten und Services. <!---

Für den {{site.data.keyword.composeForPostgreSQL}}-Service ist ein **gebührenpflichtiger** Standardplan erforderlich. Ein {{site.data.keyword.Bluemix_notm}}-Guthaben in Höhe von 200 US-Dollar kann durch Umwandlung in ein gebührenpflichtiges Konto mit einer Kreditkarte erworben werden. Wenn Sie bereits über ein gebührenpflichtiges Konto verfügen, erhalten Sie eine einmalige Rückerstattung der Kosten für Ihr erstes GB an Speicherplatz für einen Monat in Höhe von 16 US-Dollar.
{: tip}
...
1.  Melden Sie sich mit Ihrer {{site.data.keyword.ibmid}} bei Ihrem [{{site.data.keyword.Bluemix_notm}}-Konto an ![Symbol für externen Link](../../icons/launch-glyph.svg "Symbol für externen Link")](https://{DomainName}){: new_window}.
1.  Erstellen Sie für jeden der folgenden Services, der noch nicht mit Ihrem Konto verknüpft ist, eine Instanz. Klicken Sie dazu auf den Link, geben Sie dem Service einen Namen, wählen Sie den (kostenlosen) **Lite**-Plan (kostenlosen) Plan aus und klicken Sie auf die Schaltfläche **Erstellen**.
...
### Db2 Warehouse-Service einrichten
{: #gs-provdb2}

- [Richten Sie einen Db2 Warehouse-Service ein ![Symbol für externen Link](../../icons/launch-glyph.svg "Symbol für externen Link")](https://{DomainName}/catalog/services/db2-warehouse){: new_window}, sofern noch kein solcher zu Ihrem Konto zugeordnet ist: 
  ![Db2 Warehouse](images/db2_warehouse.png)

- Benennen Sie Ihren Service, wählen Sie den Einstiegsplan aus und klicken Sie auf die Schaltfläche **Erstellen**.

### Trainingsdaten in Db2 Warehouse hochladen
{: #gs-traindb2}

- Laden Sie die Datei [drug_train_data_updated.csv ![Symbol für externen Link](../../icons/launch-glyph.svg "Symbol für externen Link")](https://raw.githubusercontent.com/watson-developer-cloud/doc-tutorial-downloads/master/ai-openscale/drug_train_data_updated.csv) herunter. Stellen Sie sicher, dass Sie sich heruntergeladene Datei als CSV-Datei speichern.

- Öffnen Sie Ihre vorhandene (oder neu erstellte) Db2 Warehouse-Instanz von der [IBM Cloud-Konsole ![Symbol für externen Link](../../icons/launch-glyph.svg "Symbol für externen Link")](https://{DomainName}){: new_window} aus, klicken Sie im Seitenfenster links auf **Verwalten** und klicken Sie dann auf die Schaltfläche **Öffnen**.
- Verwenden Sie für die Anmeldung bei Db2 Warehouse bei Bedarf Ihre Db2-Berechtigungsnachweise für 'Benutzername' und 'Kennwort'.

- Nachdem Db2 Warehouse geöffnet worden ist, klicken Sie auf die Schaltfläche **Menü** und wählen Sie im Menü die Option **Laden** aus:
  ![Menü 'Laden'](images/db2_load.png)
...
1.  Als Nächstes müssen Sie eine Datenbank auswählen. Dabei stehen die folgenden beiden Optionen zur Auswahl: die Datenbank für den kostenlosen Lite-Plan oder eine vorhandene oder neue Datenbank. Wählen Sie für dieses Lernprogramm die Kachel **Datenbank des kostenlosen Lite-Plans verwenden** aus.
    Ausführlichere und detaillierte Informationen zu jeder dieser Optionen enthält der Abschnitt [Datenbank angeben](/docs/services/ai-openscale?topic=ai-openscale-connect-db). Bei der vorhandenen Datenbank kann es sich um eine PostgreSQL-Datenbank oder eine Db2-Datenbank handeln.
    {: tip}
...
Bevor Sie Ihre Überwachungen konfigurieren können, müssen Sie mindestens eine Scoring-Anforderung für Ihr Modell generieren, um eine Protokollierung von Nutzdaten zu generieren, die von den Überwachungen verbraucht werden kann. In diesem Abschnitt stellen Sie Stichprobendaten in Form einer JSON-Datei zur Verfügung, um eine Scoring-Anforderung zu generieren.
...
1.  Wählen Sie die Kachel **Für Überwachung vorbereiten** aus und klicken Sie dann auf **Beginnen**.
...
Wenn Sie das Ergebnis sofort überprüfen möchten, wählen Sie auf der Seite **Einsichten** eine Bereitstellung aus und klicken Sie dann auf die Schaltfläche **Fairness jetzt überprüfen** oder **Genauigkeit jetzt überprüfen**.
...
1.  Wählen Sie eine Bereitstellung aus, indem Sie auf der Seite 'Einsichten' auf die entsprechende Kachel klicken. Es werden die Überwachungsdaten für diese Bereitstellung angezeigt. Hinweis: Nachdem Sie die CSV-Datei mit Rückmeldedaten hochgeladen haben, kann es sein, dass die Fairness- oder Genauigkeitsdaten nicht aktualisiert werden. Wenn Sie das Ergebnis sofort überprüfen möchten, klicken Sie auf die Schaltfläche **Fairness jetzt überprüfen** oder **Genauigkeit jetzt überprüfen**.
