...
No se requieren comillas dobles (") cuando los nombres de columna están en mayúsculas, lo que hace que para Db2 no haya distinción de mayúsculas y minúsculas; sin embargo, para otras bases de datos y en la instancia donde los nombres de columna están en mayúsculas y minúsculas, estas deben coincidir.
 
Se espera que el archivo CSV de opinión tenga los valores de todas las características, y el valor de destino/etiqueta asignado manualmente. Por ejemplo, los datos de entrenamiento del modelo de fármacos contienen los valores de características `AGE`, `SEX`, `BP`, `CHOLESTEROL`,`NA`,`K` y el valor de destino/etiqueta `DRUG`. El archivo CSV de opinión debe incluir valores para estos campos; un ejemplo sería `[43, M, ALTO, NORMAL, 0.6345, 1.4587, FármacoX]`. Si se proporciona una cabecera para el archivo CSV de opinión, los nombres de campo se correlacionan utilizando esa cabecera. De lo contrario, el orden de los campos **DEBE** ser exactamente el mismo que en el esquema de entrenamiento. Para obtener más información sobre los datos de entrenamiento, consulte [¿Por qué {{site.data.keyword.aios_short}} necesita acceder a mis datos de entrenamiento?](/docs/services/ai-openscale?topic=ai-openscale-trainingdata#trainingdata) 
...
Si el archivo contiene nombres de columna, estas no tienen que coincidir necesariamente con el orden de la tabla; sin embargo, si el archivo no tiene nombres de columna, sí que debe haber coincidencia. Es posible que tenga columnas que no estén en los datos de entrenamiento originales. Estas columnas se ignoran durante el proceso. En el ejemplo siguiente se muestra un
archivo de formato CSV con formato válido donde se utilizan comillas dobles (") para los nombres de columna:
...
El formato JSON consta de una colección de objetos con campos correspondientes a los nombres de columna. En el ejemplo siguiente se muestra un archivo
de formato JSON completo y con formato válido:
...
## Carga de los datos de opinión
{: #fmt-upld-fdbk-data-uploading}

Puede cargar datos de opinión desde un archivo CSV directamente en la interfaz de usuario de {{site.data.keyword.aios_short}}. Para cargar un archivo JSON, puede utilizar {{site.data.keyword.DSX}}. 

### Carga de un archivo CSV
{: #fmt-upld-fdbk-data-upld-csv}

Para cargar un archivo CSV, utilice el botón **Añadir datos de opiniones**. Para seguir los pasos siguientes como parte de la guía de aprendizaje, abra y copie el contenido del archivo [`credit_feedback_data.csv`](https://raw.githubusercontent.com/watson-developer-cloud/doc-tutorial-downloads/master/ai-openscale/credit_feedback_data.csv).

1. En el panel de control de {{site.data.keyword.aios_short}}, pulse el mosaico de despliegue.
2. En la ventana de despliegue de modelo, pulse **Supervisores de configuración** ![Se muestra el botón de configuración de despliegue](images/configure-deployment-button.png).
3. En el panel de navegación, pulse **Calidad**.
4. Pulse **Opiniones** y a continuación pulse el botón **Añadir datos de opiniones**.
5. Seleccione el archivo CSV que contiene los datos de opiniones y pulse **Abrir**. Para la guía de aprendizaje, seleccione el archivo `credit_feedback_data.csv` que ha descargado.

    Los tamaños de archivo están actualmente limitados a 8 MB.
    {: note}

4. En el menú desplegable, pulse el delimitador de campo y pulse **Seleccionar**.

La adición del archivo CSV proporciona datos de opinión al modelo.
### Carga de un archivo JSON
{: #fmt-upld-fdbk-data-upld-json}

1. Inicie {{site.data.keyword.DSX}} y vaya al proyecto que contenga el modelo.
2. Descargue el archivo JSON.
1. En la pestaña **Despliegues** del proyecto de {{site.data.keyword.DSX}}, pulse el enlace **modelo**, pulse la pestaña **Probar** y seleccione el icono de entrada de JSON.

    ![Prueba de JSON](images/json_test02.png)

1.  Ahora, abra el archivo JSON que ha descargado y copie el contenido en el archivo JSON en la pestaña **Probar**. Pulse el botón **Predecir** para enviar y puntuar cargas útiles de entrenamiento al modelo.
