...
Per il servizio {{site.data.keyword.composeForPostgreSQL}}, è richiesto un piano standard **a pagamento**. È possibile ottenere un credito {{site.data.keyword.Bluemix_notm}} pari a 200 dollari passando a un account a pagamento con carta di credito. Se si dispone già di un account a pagamento, si riceverà un rimborso una tantum pari a 16 dollari del costo del primo GB di storage, per un mese.
{: suggerimento}
...
### Eseguire il provisioning di un servizio Db2 Warehouse
{: #gs-provdb2}

- [Eseguire il provisioning di un servizio Db2 Warehouse ![External link icon](../../icons/launch-glyph.svg "icona link esterno")](https://{DomainName}/catalog/services/db2-warehouse){: new_window} se non si dispone già di un servizio associato all'account:

  ![Db2 Warehouse](images/db2_warehouse.png)

- Fornire un nome per il servizio, selezionare il piano Entry e fare clic sul pulsante **Crea**.

### Caricare i dati di training su Db2 Warehouse
{: #gs-traindb2}

- Scaricare il file [drug_train_data_updated.csv ![External link icon](../../icons/launch-glyph.svg "icona link esterno")](https://raw.githubusercontent.com/watson-developer-cloud/doc-tutorial-downloads/master/ai-openscale/drug_train_data_updated.csv). Accertarsi di salvare il file scaricato come file .CSV.

- Aprire il Db2 Warehouse esistente (o appena creato) dalla [console IBM Cloud ![External link icon](../../icons/launch-glyph.svg "icona link esterno")](https://{DomainName}){: new_window}, fare clic su **Gestisci** dal pannello a sinistra, quindi fare clic sul pulsante **Apri**.

- Se necessario, utilizzare le credenziali Db2 `username` e `password` per accedere a Db2 Warehouse.

- Una volta aperto Db2 Warehouse, fare clic sul pulsante **Menu** e selezionare **Carica** dal menu:

  ![Load Menu](images/db2_load.png)

- Carcare il file di dati di training o trascinarlo e rilasciarlo nell'area appropriata del modulo. Fare clic su **Avanti**. Selezionare uno schema dall'elenco di destinazioni di caricamento; di norma, è in un formato simile a `DASH12345`. Quindi, fare clic su **Nuova tabella** a destra:

  ![New Table](images/new_table.png)

- Fornire un nome per la tabella HEART\_DRUG\_TRAINING e fare clic sul pulsante **Crea**:

  ![New Table Create](images/new_table_create1.png)

- Fare clic su **Avanti** per visualizzare un'anteprima dei dati. Nella schermata di anteprima, impostare il campo **Separatore** su un punto e virgola (;) e accertarsi che l'opzione **Intestazione nella prima riga** sia selezionata:
...
- Dalla scheda impostazioni del progetto, selezionare nuovamente **Aggiungi servizio** e selezionare **Spark** dal menu. Dalla scheda **Esistente**, selezionare il servizio Spark creato e fare clic su **Seleziona**.
