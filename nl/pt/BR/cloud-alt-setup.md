...
1. [Crie uma chave de API do {{site.data.keyword.cloud_notm}} e faça download dele](/docs/iam?topic=iam-userapikey#create_user_key){: external}. Será necessário inserir a chave de API em uma etapa posterior.
...
- Cada implementação é mostrada como um ladrilho. Se estiver seguindo algum dos tutoriais, no seu painel, você verá uma implementação chamada `GermanCreditRiskModel`. A amostra a seguir mostra um painel com muitos modelos implementados e monitorados:

    ![Painel de insights com muitas implementações, cada uma exibida como um bloco](images/insight-dashboard.png)
...
2. Deslize o marcador no gráfico para visualizar um período de dia e horário que mostra os dados e, em seguida, clique no link **Visualizar detalhes**.
   ...
   - Para obter informações sobre como interpretar o gráfico de séries temporais, consulte [Obtendo insights](/docs/services/ai-openscale?topic=ai-openscale-it-ov).
    ...
    ![Os dados do monitor são mostrados](images/setup02-0206.png)
...
3. Para ver detalhes sobre o monitoramento de dados `AGE`, assegure-se de que `AGE` esteja selecionado no menu suspenso.
  ...
  - Para obter informações sobre como interpretar o gráfico dos pontos de dados em uma hora específica, consulte [Visualizando dados para uma hora específica](/docs/services/ai-openscale?topic=ai-openscale-it-vdet).

    ![Os detalhes da visualização são mostrados](images/setup03-0206.png)
...
- Para saber mais sobre a interpretação de gráficos e dados, consulte [Obtendo insights](/docs/services/ai-openscale?topic=ai-openscale-it-ov).
